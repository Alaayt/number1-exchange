...
const STATUS_CONFIG = {
  pending:    { labelAr: 'في الانتظار',   labelEn: 'Pending',     color: '#f59e0b', icon: <_IcClock  />, descAr: 'في انتظار استلام دفعتك',       descEn: 'Waiting to receive your payment',     steps: [true,  false, false, false] },
  verifying:  { labelAr: 'جاري التحقق',  labelEn: 'Verifying',   color: '#a78bfa', icon: <_IcSearch />, descAr: 'يراجع الفريق دفعتك',           descEn: 'Team is reviewing your payment',      steps: [true,  true,  false, false] },
  verified:   { labelAr: 'تم التحقق',    labelEn: 'Verified',    color: '#60a5fa', icon: <_IcSearch />, descAr: 'تم التحقق من الدفع',           descEn: 'Payment verified',                    steps: [true,  true,  false, false] },
  processing: { labelAr: 'قيد المعالجة', labelEn: 'Processing',  color: '#00b8d9', icon: <_IcZap    />, descAr: 'جاري إرسال المبلغ',            descEn: 'Amount is being sent',                steps: [true,  true,  true,  false] },
  completed:  { labelAr: 'مكتمل',        labelEn: 'Completed',   color: '#00e5a0', icon: <_IcCheck2 />, descAr: 'تم إرسال المبلغ بنجاح',       descEn: 'Amount sent successfully',            steps: [true,  true,  true,  true]  },
  rejected:   { labelAr: 'مرفوض',        labelEn: 'Rejected',    color: '#f43f5e', icon: <_IcXCircle/>, descAr: 'تم رفض الطلب، تواصل مع الدعم', descEn: 'Order rejected, contact support',     steps: [true, false, false, false] },
  cancelled:  { labelAr: 'ملغي',         labelEn: 'Cancelled',   color: '#6e7681', icon: <_IcXCircle/>, descAr: 'تم إلغاء الطلب',               descEn: 'Order cancelled',                     steps: [true, false, false, false] },
}

const TRACK_STEPS_AR = ['استلام الدفعة', 'مراجعة الطلب', 'معالجة التحويل', 'اكتمال الإرسال']
const TRACK_STEPS_EN = ['Payment Received', 'Order Review', 'Processing Transfer', 'Transfer Complete']

const ORDER_TYPE_LABELS = {
  USDT_TO_MONEYGO:       'USDT → MoneyGo USD',
  EGP_WALLET_TO_MONEYGO: { ar: 'محفظة مصرية → MoneyGo USD', en: 'Egyptian Wallet → MoneyGo USD' },
}

function StatusBadge({ status, isEn }) {
  const cfg = STATUS_CONFIG[status] || STATUS_CONFIG.pending
  const label = isEn ? cfg.labelEn : cfg.labelAr
  return (
...
function ProgressTracker({ steps, isEn }) {
  const labels = isEn ? TRACK_STEPS_EN : TRACK_STEPS_AR
  return (
...
export default function OrderTrack() {
  ...
  const doSearch = async (idOverride) => {
    ...
    try {
      ...
      setError(isEn ? 'Could not connect to server. Please try again.' : 'تعذر الاتصال بالخادم. حاول مرة أخرى.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const getOrderTypeLabel = (type) => {
    const lbl = ORDER_TYPE_LABELS[type]
    if (!lbl) return type
    if (typeof lbl === 'string') return lbl
    return isEn ? lbl.en : lbl.ar
  }

  return (
    <div style={{ minHeight:'80vh', padding:'60px 24px', maxWidth:700, margin:'0 auto', direction: isEn ? 'ltr' : 'rtl' }}>
      ...
      <div style={{ textAlign:'center', marginBottom:48 }}>
        ...
        <p style={{ fontSize:'1rem', color:'var(--text-3)', fontFamily:"'Tajawal',sans-serif", lineHeight:1.8, maxWidth:380, margin:'0 auto' }}>
          {isEn ? 'Enter your order number to check its current status' : 'أدخل رقم طلبك لمعرفة الحالة الحالية'}
        </p>
      </div>

      {/* Search box */}
      <div style={{ background:'var(--card)', border:'1px solid var(--border-1)', borderRadius:20, padding:'28px 24px', marginBottom:28 }}>
        <label style={{ display:'block', fontSize:'0.72rem', color:'var(--text-3)', fontFamily:"'JetBrains Mono',monospace", marginBottom:10, letterSpacing:1 }}>{isEn ? 'Order Number' : 'رقم الطلب'}</label>
      ...
      {notFound && (
        <div style={{ textAlign:'center', padding:'32px', background:'var(--card)', border:'1px solid rgba(244,63,94,0.3)', borderRadius:16 }}>
          ...
          <h3 style={{ fontFamily:"'Tajawal',sans-serif", color:'var(--text-1)', margin:'0 0 8px' }}>{isEn ? 'Order Not Found' : 'الطلب غير موجود'}</h3>
          <p style={{ color:'var(--text-3)', fontFamily:"'Tajawal',sans-serif", fontSize:'0.86rem' }}>
            {isEn
              ? <>Check the order number or contact <a href="/contact" style={{ color:'var(--cyan)' }}>support</a></>
      ...
      {result && cfg && (
        <div style={{ background:'var(--card)', border:`1px solid ${cfg.color}40`, borderRadius:20, padding:'28px 24px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginTop:24 }}>
            {[
              { labelAr: 'نوع الطلب',      labelEn: 'Order Type',      value: getOrderTypeLabel(result.orderType) },
              { labelAr: 'المبلغ المرسل',  labelEn: 'Amount Sent',     value: `${result.payment?.amountSent} ${result.payment?.currencySent}` },
              { labelAr: 'المبلغ المستلَم', labelEn: 'Amount Received', value: `${result.moneygo?.amountUSD} USD` },
              { labelAr: 'تاريخ الطلب',    labelEn: 'Order Date',      value: new Date(result.createdAt).toLocaleString(isEn ? 'en-US' : 'ar-EG') },
            ].map(d => (
              <div key={isEn ? d.labelEn : d.labelAr} style={{ background:'rgba(255,255,255,0.02)', border:'1px solid var(--border-1)', borderRadius:10, padding:'12px 14px' }}>
                <div style={{ fontSize:'0.68rem', color:'var(--text-3)', fontFamily:"'JetBrains Mono',monospace", marginBottom:4, letterSpacing:1 }}>{isEn ? d.labelEn : d.labelAr}</div>
          ...
          {result.timeline?.length > 0 && (
            <div style={{ marginTop:20 }}>
              <div style={{ fontSize:'0.72rem', color:'var(--text-3)', fontFamily:"'JetBrains Mono',monospace", marginBottom:10, letterSpacing:1 }}>{isEn ? 'Event Log' : 'سجل الأحداث'}</div>
                ...
                <div key={i} style={{ display:'flex', gap:12, padding:'8px 0', borderBottom:'1px solid var(--border-1)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:'0.68rem', color:'var(--text-3)', fontFamily:"'JetBrains Mono',monospace", marginTop:2 }}>
                      {new Date(t.timestamp).toLocaleString(isEn ? 'en-US' : 'ar-EG')}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div style={{ marginTop:18, padding:'12px 16px', borderRadius:12, background: cfg.color+'10', border:`1px solid ${cfg.color}25` }}>
            <span style={{ fontSize:'0.85rem', color: cfg.color, fontFamily:"'Tajawal',sans-serif" }}>{cfg.icon} {isEn ? cfg.descEn : cfg.descAr}</span>
          </div>

          {result.status !== 'completed' && (
            <p style={{ margin:'16px 0 0', textAlign:'center', fontSize:'0.8rem', color:'var(--text-3)', fontFamily:"'Tajawal',sans-serif" }}>
              {isEn
                ? <>Need help? <a href="/contact" style={{ color:'var(--cyan)', textDecoration:'none' }}>Contact support</a></>
