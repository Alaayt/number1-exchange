...
function Panel({ onClose, lang }) {
  ...
  useEffect(()=>{
    ...
    setTimeout(()=>{
      ...
      setRobAnim('blink')
      setTimeout(()=>{ setRobAnim('idle'); setShowOpts(true) },700)
    },1100)
  },[tab])
  ...
  const pickQ=(id,text)=>{
    setShowOpts(false);setShowSup(false);addMsg(text,true)
    setTyping(true);setRobAnim('talking')
    setTimeout(()=>{
      setTyping(false); const ans=BOT_ANS[ar?'ar':'en'][id]; setRobAnim('idle'); addMsg(ans,false,'idle')
      setTimeout(()=>{ addMsg(ar?'هل هناك شيء آخر أقدر أساعدك به؟':'Anything else I can help you with?',false,'blink'); setTimeout(()=>setShowOpts(true),300) },600)
    },900+Math.random()*400)
  }

  const sendFree=()=>{
    if(!input.trim())return
    const txt=input.trim();setInput('');setShowOpts(false);setShowSup(false);addMsg(txt,true)
    setTyping(true);setRobAnim('talking')
    setTimeout(()=>{
    ...
    <div style={{ position:'fixed', bottom:92, left:22, zIndex:500, width:355, maxHeight:580, background:'var(--card)', border:'1px solid rgba(0,210,255,0.18)', borderRadius:24, overflow:'hidden', boxShadow:'0 28px 70px rgba(0,0,0,.7), 0 0 0 1px rgba(0,210,255,0.06)', display:'flex', flexDirection:'column', animation:'n1SlideUp .28s cubic-bezier(.22,1,.36,1)' }}>
      ...
      {tab==='chat' && (
        <>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:7, marginBottom:10 }}>
                {qs.map((q,i)=>(
                  <button key={q.id} onClick={()=>pickQ(q.id,q.text)}
                    style={{ padding:'9px 14px', borderRadius:12, background:'transparent', border:'1px solid rgba(0,210,255,0.28)', color:'var(--cyan)', fontSize:'.82rem', cursor:'pointer', textAlign:'right', fontFamily:"'Tajawal',sans-serif", transition:'all .2s', animation:`n1BtnIn .2s ease ${i*.06}s both` }}
                    onMouseEnter={e=>{e.currentTarget.style.background='rgba(0,210,255,0.09)';e.currentTarget.style.borderColor='var(--cyan)'}}
                    onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.borderColor='rgba(0,210,255,0.28)'}}>
                    {q.text}
                  </button>
                ))}
              </div>
          ...
          <div style={{ padding:'10px 14px', borderTop:'1px solid rgba(0,210,255,0.1)', display:'flex', gap:8, alignItems:'center', background:'var(--card)', flexShrink:0 }}>
            ...
            <button onClick={sendFree} style={{ width:38, height:38, borderRadius:11, background:'linear-gradient(135deg,#00c2ec,#007ec7)', border:'none', cursor:'pointer', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s', boxShadow:'0 3px 12px rgba(0,210,255,0.3)' }}
    ...
    <>
      ...
      <div style={{ position:'fixed', bottom:24, left:22, zIndex:500 }}>
        <button onClick={toggle} style={{ width:62, height:62, borderRadius:'50%', background:open?'linear-gradient(135deg,#ff3d5a,#b0002a)':'linear-gradient(135deg,#00d2ff,#005fa3)', border:'2px solid rgba(255,255,255,0.18)', cursor:'pointer', padding:0, overflow:'hidden', boxShadow:open?'0 6px 24px rgba(255,61,90,.6)':'0 6px 24px rgba(0,210,255,.6)', animation:!open?'n1FabPulse 2.5s infinite':'none', transition:'background .3s, box-shadow .3s', display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
          ...
          {!open && unread>0 && (
            <div style={{ position:'absolute', top:-2, right:-2, width:20, height:20, borderRadius:'50%', background:'#ff3d5a', color:'#fff', fontSize:'.63rem', fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', border:'2px solid var(--bg)', fontFamily:"'JetBrains Mono',monospace" }}>{unread}</div>
          )}
        </button>
      </div>
    </>
