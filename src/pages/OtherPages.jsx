// src/pages/OtherPages.jsx — News + Support + About في ملف واحد
  ...
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose()}}
      ...
      <div style={{ background:'var(--card)',border:'1px solid var(--border-2)',borderRadius:22,width:'100%',maxWidth:600,maxHeight:'80vh',overflow:'hidden',display:'flex',flexDirection:'column',position:'relative' }}>
        ...
        <div style={{ padding:22,overflowY:'auto',fontSize:'0.9rem',color:'var(--text-2)',lineHeight:1.85 }}>{lang==='ar'?article.bodyAr:article.bodyEn}</div>
      </div>
    </div>
  )
}
      ...
      <div style={{ maxWidth:1200,margin:'0 auto',padding:'55px 22px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:18 }}>
          {NEWS.map((article,i)=>{
            ...
            return (
              <div key={i} onClick={()=>setSelected(article)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
                ...
                <div style={{ height:120,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'2.5rem',borderBottom:'1px solid var(--border-1)',background:'linear-gradient(135deg,rgba(0,210,255,0.08),rgba(124,92,252,0.08))' }}>{article.icon}</div>
...
export function Support() {
  ...
  const { lang } = useLang()
  const contacts = [
    { icon:'🤖', titleKey:'support_bot', descKey:'support_bot_desc', onClick:()=>{}                                          },
    { icon:'✈️', titleKey:'support_tg',  descKey:'support_tg_desc',  onClick:()=>window.open('https://t.me/Number1Exchange','_blank') },
    { icon:'📱', titleKey:'support_wa',  descKey:'support_wa_desc',  onClick:()=>window.open('https://wa.me/967700000001','_blank')   },
  ]
  return (
    <div style={{ position:'relative',zIndex:2 }}>
      <div style={{ maxWidth:1200,margin:'0 auto',padding:'55px 22px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:18,marginBottom:40 }}>
          {contacts.map((c,i)=>{
            const [hov,setHov]=useState(false)
            return (
              <div key={i} onClick={c.onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
                ...
                <div style={{ width:56,height:56,borderRadius:16,background:'var(--cyan-dim)',border:'1px solid rgba(0,210,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px',fontSize:'1.5rem',transition:'transform 0.3s',transform: hov?'scale(1.1) rotate(-5deg)':'none' }}>{c.icon}</div>
                <h3 style={{ fontSize:'0.95rem',fontWeight:700,marginBottom:6 }}>{t(c.titleKey)}</h3>
                <p style={{ fontSize:'0.8rem',color:'var(--text-2)' }}>{t(c.descKey)}</p>
              </div>
            )
          })}
        </div>
          ...
          <div style={{ padding:'0 22px' }}>
            {FAQS.map((f,i)=>(
              <div key={i} style={{ borderBottom:'1px solid var(--border-1)' }}>
                ...
                {openFaq===i && <div style={{ fontSize:'0.85rem',color:'var(--text-2)',lineHeight:1.75,paddingBottom:14 }}>{lang==='ar'?f.aAr:f.aEn}</div>}
              </div>
            ))}
          </div>
        </div>
...
const STATS = [
  ...
  { icon:'🌍', titleAr:'50+ دولة',  titleEn:'50+ Countries',descAr:'تغطية جغرافية واسعة في المنطقة العربية والعالم', descEn:'Wide geographic coverage in the Arab region and globally' },
]
const TEAM = [
    ...
    <div style={{ position:'relative',zIndex:2 }}>
      <div style={{ maxWidth:1200,margin:'0 auto',padding:'55px 22px' }}>
          ...
          {STATS.map((s,i)=>{
            ...
            return (
              <div key={i} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
                ...
                <h3 style={{ fontFamily:"'Orbitron',sans-serif",fontSize:'1.2rem',fontWeight:900,color:'var(--cyan)',marginBottom:8,textShadow:'0 0 18px rgba(0,210,255,0.4)' }}>{lang==='ar'?s.titleAr:s.titleEn}</h3>
                <p style={{ fontSize:'0.82rem',color:'var(--text-2)',lineHeight:1.6 }}>{lang==='ar'?s.descAr:s.descEn}</p>
              </div>
            )
          })}
        </div>
        ...
        <div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:18 }}>
            {TEAM.map((m,i)=>(
              <div key={i} style={{ background:'var(--card)',border:'1px solid var(--border-1)',borderRadius:20,padding:'28px 22px',textAlign:'center',transition:'all 0.3s' }}
                ...
                <div style={{ width:72,height:72,borderRadius:'50%',background:m.color,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px',fontSize:'1.8rem',fontWeight:900,color:'#fff' }}>{lang==='ar'?m.nameAr[0]:m.nameEn[0]}</div>
                <h3 style={{ fontSize:'1rem',fontWeight:800,marginBottom:6 }}>{lang==='ar'?m.nameAr:m.nameEn}</h3>
                <p style={{ fontSize:'0.82rem',color:'var(--text-2)' }}>{lang==='ar'?m.roleAr:m.roleEn}</p>
              </div>
            ))}
          </div>
        </div>
